...
{  
    unsigned long h[SHA1_DIGEST_SIZE / 4]; /* 存放摘要结果(32*5=160 bits)*/  
    unsigned long Nl;  
...
int SHA1_String(const unsigned char* inputString, unsigned long len, char szSha1Digest[40],bool bLowerCase)
{
	int rt = -1;  
	unsigned char szDigest_temp[40] = {0};
	char szDigestTemp[4] = {0};

    SHA1_Context c;  
	
    if ((NULL == inputString) || (NULL == szSha1Digest))  
    {  
        return -1;  
    }  
    ...
    SHA1_Final(&c,szDigest_temp);  
    SHA1_Clear_data(&c);  

	for (int index = 0; index < SHA1_DIGEST_SIZE; index++)
	{		
		if (bLowerCase)
		{
			sprintf_s(szDigestTemp,4,"%02x",(int)szDigest_temp[index]);
		}
		else
		{
			sprintf_s(szDigestTemp,4,"%02X",(int)szDigest_temp[index]);
		...
		strcat_s(szSha1Digest,80,szDigestTemp);
	}
